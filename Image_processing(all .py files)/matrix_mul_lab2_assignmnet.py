...
f, axis = plt.subplots(2, 3, figsize=(20,20))
...
axis[0,2].imshow(img,'gray')
axis[0,2].set_title('Resized Gray(as input)')

axis[1,0].imshow(out,'gray')
axis[1,0].set_title('Output(Using matrix_multiplication)')

result1 = signal.convolve2d(img, kernel, "full")/255
result1 = cv2.cvtColor(result1, cv2.COLOR_BGR2RGB)
axis[1,1].imshow(result1,'gray')
axis[1,1].set_title('Using Convolve2d')

result2 = cv2.filter2D(img, -1, kernel)
result2 = cv2.cvtColor(result2, cv2.COLOR_BGR2RGB)
axis[1,2].imshow(result2,'gray')
axis[1,2].set_title('Using filter2D')